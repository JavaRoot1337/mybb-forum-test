...
export MYBB_INSTALL_DB_PATH="${containerWorkspaceFolder}/public/db.sqlite"
...
if [ -f config/composer.json ]; then
    composer install --working-dir=config
fi
...
if [[ -f bin/cli ]]; then
    ...
    if [[ $(php bin/cli status --code) != '3' ]]; then
        touch public/inc/config.php public/inc/settings.php
